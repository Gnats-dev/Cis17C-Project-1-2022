 ...
 *
 * Created on April 28, 2022, 12:47 AM
 */
...
int main(int argc, char** argv) {
    ...
    //Print the deck and then shuffle it.
    deck->prntDeck();
    
    cout << endl << "Shuffling..." << endl;
    cout << "The deck has now been shuffled." << endl << endl;
    ...
    cout << "Welcome to Blackjack, " << player->getName() <<". Would you like to see the game rules?" << endl;
    ...
    if (seeRule == 'Y' || seeRule == 'y') {
        do {
            ...
            cout << "2. The player will receive 2 cards and the dealer will reveal one of his cards." << endl;
            ...
            cout << "7. You win when the combined value of your cards is greater than that of the dealers, while still being below 22." << endl;
    ...
    cout << endl <<  "Wish you luck, and enjoy the game!" << endl;
    ...
    do {
        ...
        //a valid input.
        //Variable "valid" of type bool asserts whether the input is valid. If it is not, none of the alphabetic case is met and it falls to default,
        ...
        //Displaying both of their bank after making a wager. Dealer's is untouched, while player's is subtracted from it
        //the bet he has to make.
        cout << "Player's bank: $" << player->getMoney() << endl;
        
        //The dealer's bank is $1600.
        ...
        //If none of them has lost the game yet. In other words, the player survives the loop with possibly a BJ or a hand less than 21, the dealer must adopt a strategy.
        while(!player->getLose() && !dealer->getLose() && !draw) {
            //None of them has lost, and they didn't end up getting the same score.
            ...
            if(dealer->getHandScore() < player->getHandScore()) {
                ...
                //The dealer gets BJ and the player doesn't. The dealer wins.
                else if((dealer->getHandScore() == 21) && (player->getHandScore() != 21)) {
                    dealer->setWin();
                    cout << "Dealer gets BLACKJACK!! You lose!" << endl;
                    dealer->winMoney(player->getWager());
                }
                //The dealer gets a winning hand. The player loses.
                else if(dealer->getHandScore() > player->getHandScore()) {
                    dealer->setWin();
                    cout << "You lose!" << endl;
                    dealer->winMoney(player->getWager());
                }
                //They both get 21. It's a draw. Player is returned his money
                else if((dealer->getHandScore() == 21) && (player->getHandScore() == 21)) {
                    draw = true;
                    cout << "You and the dealer both get BLACKJACK. **Draw**." << endl; 
            ...
            //If non of them gets BJ and they end up with same scores, it is a draw. 
                ...
                draw = true;
                cout << "**Draw**. This is a close one." << endl; 
            //Nobody wins, nobody loses. Player gets his wager back into his bank.
                player->winMoney(player->getWager());
            }
        }
        
        //Set hidden to true and clear both of player's and dealer's hands, so if the player decides to go for another round, he will be getting a new hand. These are restart conditions.
        ...
        //If the player doesn't have any money in his bank, the game terminates; vice versa
        if(player->getMoney() <= 0) {
            cout << "You have run out of cash. You lose." << endl;
            again = 'n';
        }
        else if(dealer->getMoney() <= 0) {
            cout << "The Dealer has run out of money. You have cleaned him out!" << endl;
            again = 'n';
        }
        //If there's no card left in the deck, the game is over.
        else if(deck->getCardsCount() <= 0) {
            cout << "There are no cards left in the deck! Game over." << endl;
        }
    ...
    //Clearing up memory allocation at the end.
    delete deck;
    delete player;
    delete dealer;
